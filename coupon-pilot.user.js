...
(async function CouponPilot() {
  ...
  function registerModule(module) {
    const required = ['id', 'name', 'matches', 'discoverItems', 'perform', 'verify', 'healthCheck'];
    const missing = required.filter(key => !module[key]);
    if (missing.length) throw new Error(`Invalid Coupon Pilot module: ${missing.join(', ')}`);
    modules.push(module);
  }
  ...
  function enabledBlockedTerms(module) {
    ...
    for (const [groupName, groupTerms] of Object.entries(module.defaultBlockedGroups || {})) {
      if (moduleState.enabledGroups[groupName]) terms.push(...groupTerms);
    }
  ...
  function evaluateRules(item, module) {
    ...
    const blocked = enabledBlockedTerms(module).find(term => haystack.includes(term));
  ...
  function classify(items, module) {
    return items.map(item => ({ ...item, decision: evaluateRules(item, module) }));
  }
  ...
  registerModule({
    ...
    matches() {
      return location.hostname === 'www.harristeeter.com' && /\/coupons(?:\/|$)/i.test(location.pathname);
    },

    isCouponControl(element) {
      if (!visible(element)) return false;
      const label = `${textOf(element)} ${normalize(element.getAttribute('aria-label'))}`;
      return /\bclip\b/i.test(label) || /\bclipped\b/i.test(label) || /\bunclip\b/i.test(label);
    },

    controlStatus(element) {
      if (!visible(element)) return 'ambiguous';

      const visibleText = textOf(element);
      const label = `${visibleText} ${normalize(element.getAttribute('aria-label'))}`;

      if (/\bclipped\b|\bunclip\b|\bremove coupon\b/i.test(label)) return 'clipped';

      const explicitClip = /^clip$/i.test(visibleText) || /\bclip coupon\b/i.test(label);
      if (explicitClip && !element.disabled && element.getAttribute('aria-disabled') !== 'true') {
        return 'available';
      }
    ...
    couponControlsWithin(element) {
      return [...element.querySelectorAll('button, a, [role="button"]')]
        .filter(control => this.isCouponControl(control));
    },
    ...
    findCard(control) {
      const explicitSelectors = [
        ...
        '[data-qa*="coupon" i]',
        '[data-cy*="coupon" i]',
        '[data-component*="coupon" i]'
      ];

      for (const selector of explicitSelectors) {
        const candidate = control.closest(selector);
        if (!candidate) continue;
        const text = textOf(candidate);
        const controls = this.couponControlsWithin(candidate);
        if (text.length >= 20 && text.length <= 2500 && controls.length === 1) return candidate;
      }
      ...
      for (let depth = 0; depth < 8 && node; depth++, node = node.parentElement) {
        ...
        if (!/\bsave\b|\bexpires?\b|\bcoupon\b|\boff\b/i.test(text)) continue;

        const controls = this.couponControlsWithin(node);
        if (controls.length === 1 && controls[0] === control) return node;
      }
    ...
    itemId(card) {
      const direct = card?.getAttribute('data-coupon-id') || card?.getAttribute('data-offer-id');
      if (direct) return `ht:${direct}`;

      const nested = card?.querySelector('[data-coupon-id], [data-offer-id]');
      const nestedId = nested?.getAttribute('data-coupon-id') || nested?.getAttribute('data-offer-id');
      if (nestedId) return `ht:${nestedId}`;
    ...
    discoverItems() {
      ...
      for (const control of controls) {
        ...
        const title = normalize((card.innerText || card.textContent || text)
          ...
          .map(line => line.trim())
          .filter(Boolean)
          .filter(line => !/^(clip|clipped|unclip)$/i.test(line))
          .slice(0, 3)
          .join(' · ')) || text.slice(0, 140);
        ...
        if (!previous || (previous.status === 'ambiguous' && status !== 'ambiguous')) {
          byId.set(id, item);
        }
      }
    ...
    async perform(id) {
      ...
      if (!current || current.status !== 'available' || !current.control?.isConnected) {
        throw new Error('Coupon is no longer safely actionable');
      }
    ...
    async verify(id, { timeout, signal }) {
      const result = await waitFor(() => {
        const current = this.getItem(id);
        return current?.status === 'clipped' ? 'clipped' : false;
      }, {
        timeout,
        interval: 180,
        signal
    ...
    healthCheck() {
      ...
      if (available === 0 && clipped > 0 && ambiguous === 0) {
        return {
          level: 'done',
      ...
      if (ambiguous > 0) {
        return {
          level: available > 0 ? 'caution' : 'warning',
          message: available > 0
            ? `${available} ready · ${ambiguous} skipped as ambiguous`
            : `${ambiguous} ambiguous coupon control${ambiguous === 1 ? '' : 's'} detected`,
          found: items.length,
          available,
          clipped,
          ambiguous
      ...
      return {
        level: 'ready',
        message: `${available} ready to clip`,
        found: items.length,
        available,
        clipped,
        ambiguous
  ...
  function logActivity(message, level = 'info') {
    activity.unshift({ at: new Date(), message, level });
  ...
  async function executeWithRetry(id, module, signal) {
    ...
    for (let attempt = 1; attempt <= attempts; attempt++) {
      ...
      try {
        ...
        logActivity(`Retrying after verification failure (${attempt}/${attempts})`, 'caution');
  ...
  async function startRun() {
    ...
    if (initialHealth.found === 0) {
      toast('No coupon cards are detected yet. Let the page finish loading and try again.');
      return;
    }

    if (initialHealth.available === 0) {
      toast(initialHealth.ambiguous > 0
        ? 'No safely actionable coupons were detected. Use Dry run and review the page.'
        : 'No unclipped coupons are currently detected.');
      return;
    }
    ...
    try {
      while (!controller.signal.aborted &&
        ...
        for (const item of items) {
          ...
          if (!item.decision.eligible) {
            ...
            if (state.automation.dryRun) {
              item.element?.setAttribute(PREVIEW_ATTR, 'blocked');
              item.element?.style.setProperty('outline', '2px solid rgba(245,158,11,.9)');
          ...
          if (state.automation.dryRun) {
            runStats.previewed++;
            item.element?.setAttribute(PREVIEW_ATTR, 'eligible');
            item.element?.style.setProperty('outline', '2px solid rgba(99,102,241,.9)');
            item.element?.style.setProperty('outline-offset', '2px');
            ...
            try {
              ...
              logActivity(`Clipped: ${item.title}`, 'success');
            } catch (error) {
              if (error?.name === 'AbortError') throw error;
              runStats.failed++;
              consecutiveFailures++;
        ...
        const changed = await revealMore(module, controller.signal);
        const height = document.documentElement.scrollHeight;
        const known = module.discoverItems().length;

        if (progress || changed || height !== previousHeight || known !== previousKnown) idleCycles = 0;
        else idleCycles++;
      ...
      logActivity(state.automation.dryRun
        ? `Dry run complete: ${runStats.previewed} eligible · ${runStats.blocked} blocked`
      ...
      if (error?.name === 'AbortError') {
        ...
        toast('Coupon Pilot stopped because actions could not be verified reliably.');
      }
    } finally {
      running = false;
      controller = null;
  ...
  shadow.innerHTML = `
    <style>
      * { box-sizing: border-box; }
      ...
      button:focus-visible, input:focus-visible { outline: 2px solid #818cf8; outline-offset: 2px; }
      ...
      .app.minimized { width: auto; min-width: 190px; }
      .app.minimized .body, .app.minimized .footer, .app.minimized .healthRow { display: none; }
      .header { min-height: 62px; padding: 12px 14px; display: flex; align-items: center; gap: 10px; border-bottom: 1px solid rgba(255,255,255,.07); }
      .mark { width: 34px; height: 34px; display: grid; place-items: center; flex: 0 0 auto; border-radius: 11px; color: white; background: linear-gradient(135deg, #8b5cf6, #4f46e5); box-shadow: 0 8px 28px rgba(79,70,229,.35); font-size: 12px; font-weight: 850; }
      .heading { min-width: 0; flex: 1; }
      .title { font-size: 14px; font-weight: 780; letter-spacing: -.015em; }
      .subtitle { margin-top: 2px; overflow: hidden; color: #8f9bb3; font-size: 11px; white-space: nowrap; text-overflow: ellipsis; }
      .iconButton, .ghostButton, .chip, .tab, .primaryButton, .dangerButton { appearance: none; border: 0; cursor: pointer; }
      .iconButton { width: 32px; height: 32px; display: grid; place-items: center; color: #aeb8cb; background: rgba(255,255,255,.055); border: 1px solid rgba(255,255,255,.055); border-radius: 10px; }
      .iconButton:hover { background: rgba(255,255,255,.10); color: #fff; }

      .healthRow { padding: 9px 14px; display: flex; align-items: center; gap: 8px; color: #9aa6ba; background: rgba(255,255,255,.025); border-bottom: 1px solid rgba(255,255,255,.06); font-size: 11px; }
      ...
      .healthDot.caution { background: #fbbf24; box-shadow: 0 0 0 3px rgba(251,191,36,.09); }
      .healthDot.warning { background: #f97316; box-shadow: 0 0 0 3px rgba(249,115,22,.09); }
      .healthText { flex: 1; }
      ...
      .searchIcon { position: absolute; top: 10px; left: 12px; color: #69758a; pointer-events: none; }
      input[type="text"], input[type="number"] { height: 40px; color: #f8fafc; background: rgba(255,255,255,.045); border: 1px solid rgba(255,255,255,.08); border-radius: 12px; outline: none; }
      input[type="text"]:focus, input[type="number"]:focus { border-color: rgba(129,140,248,.75); box-shadow: 0 0 0 3px rgba(99,102,241,.13); }
      ...
      .stat { min-width: 0; padding: 10px 9px; background: rgba(255,255,255,.032); border: 1px solid rgba(255,255,255,.065); border-radius: 12px; }
      .statValue { color: #f8fafc; font-size: 17px; font-weight: 800; letter-spacing: -.025em; }
      .statLabel { margin-top: 2px; color: #677389; font-size: 9px; font-weight: 700; letter-spacing: .08em; text-transform: uppercase; }
      .sectionTitle { margin-bottom: 9px; display: flex; align-items: center; justify-content: space-between; color: #8e9aaf; font-size: 10px; font-weight: 760; letter-spacing: .09em; text-transform: uppercase; }
      .chips, .terms { display: flex; flex-wrap: wrap; gap: 6px; }
      .chip { height: 30px; padding: 0 11px; color: #aeb8ca; background: rgba(255,255,255,.035); border: 1px solid rgba(255,255,255,.08); border-radius: 999px; font-size: 11px; }
      .chip:hover { color: #fff; background: rgba(255,255,255,.07); }
      .chip.on { color: #ddd6fe; background: rgba(124,58,237,.17); border-color: rgba(139,92,246,.48); }
      .tabs { margin-bottom: 10px; display: grid; grid-template-columns: 1fr 1fr; gap: 4px; padding: 3px; background: rgba(255,255,255,.035); border-radius: 11px; }
      .tab { height: 30px; color: #8490a4; background: transparent; border-radius: 8px; font-size: 11px; }
      ...
      .ghostButton { height: 40px; padding: 0 13px; color: #d2d8e4; background: rgba(255,255,255,.07); border-radius: 11px; }
      .ghostButton:hover { background: rgba(255,255,255,.11); }
      .terms { margin-top: 9px; }
      .term { display: inline-flex; align-items: center; gap: 6px; max-width: 100%; padding: 5px 8px; border-radius: 8px; font-size: 11px; }
      ...
      .settingsGrid { display: grid; grid-template-columns: 1fr 1fr; gap: 9px 12px; align-items: center; }
      ...
      .couponRow, .activityRow { padding: 9px 0; border-bottom: 1px solid rgba(255,255,255,.05); }
      ...
      .activityRow.caution { color: #fde68a; }
      .activityRow.success { color: #bbf7d0; }
      .empty { padding: 8px 0; color: #667287; font-size: 11px; }
      .footer { padding: 12px 14px 14px; display: grid; grid-template-columns: 1fr 92px; gap: 8px; background: rgba(8,11,18,.94); border-top: 1px solid rgba(255,255,255,.07); }
      .primaryButton, .dangerButton { height: 42px; border-radius: 12px; font-weight: 760; }
      .primaryButton { color: white; background: linear-gradient(135deg, #7c3aed, #4f46e5); box-shadow: 0 9px 24px rgba(79,70,229,.23); }
      .primaryButton:hover { filter: brightness(1.08); }
      .primaryButton:disabled { opacity: .46; cursor: default; filter: none; }
      .dangerButton { color: #fecaca; background: rgba(239,68,68,.09); border: 1px solid rgba(239,68,68,.13); }
      .dangerButton:disabled { opacity: .3; cursor: default; }
      .toast { position: absolute; right: 0; bottom: calc(100% + 10px); width: min(360px, calc(100vw - 44px)); padding: 10px 12px; color: #f8fafc; background: #111827; border: 1px solid rgba(255,255,255,.08); border-radius: 11px; box-shadow: 0 14px 36px rgba(0,0,0,.35); opacity: 0; transform: translateY(5px); pointer-events: none; transition: opacity .16s ease, transform .16s ease; font-size: 11px; }
      .toast.show { opacity: 1; transform: translateY(0); }

      @media (max-width: 520px) {
        .app { width: calc(100vw - 24px); max-height: calc(100vh - 24px); }
        .stats { grid-template-columns: repeat(2, 1fr); }
      }
    </style>

    <div class="toast" role="status" aria-live="polite"></div>
    <div class="app" role="dialog" aria-label="Coupon Pilot">
      <div class="header">
        ...
        <button type="button" class="iconButton minimize" title="Minimize" aria-label="Minimize Coupon Pilot">−</button>
      </div>
      ...
      <div class="body">
        <div class="section">
          <div class="searchWrap">
            ...
            <input class="search" type="text" aria-label="Search loaded coupons" placeholder="Search loaded coupons…" />
          </div>
        </div>
        ...
        <div class="section">
          ...
          <div class="tabs">
            <button type="button" class="tab active" data-rule-tab="block">Never clip</button>
            <button type="button" class="tab" data-rule-tab="always">Always clip</button>
          </div>
          <div class="keywordRow">
            <input class="keywordInput" type="text" aria-label="Keyword rule" placeholder="e.g. formula" />
            <button type="button" class="ghostButton addTerm">Add</button>
          </div>
          <div class="terms"></div>
        </div>
        ...
        <div class="section">
          ...
          <div class="settingsGrid">
            ...
            <label class="setting"><span>Delay ms</span><input class="numberInput delay" type="number" min="250" max="5000" step="50" /></label>
      ...
      <div class="footer">
        <button type="button" class="primaryButton start">Run dry preview</button>
        <button type="button" class="dangerButton stop" disabled>Stop</button>
      </div>
    </div>
  `;
  ...
  function renderGroups() {
    ...
    for (const groupName of Object.keys(activeModule.defaultBlockedGroups || {})) {
      ...
      button.setAttribute('aria-pressed', String(Boolean(moduleState.enabledGroups[groupName])));
  ...
  function renderTerms() {
    ...
    const moduleState = getModuleState(activeModule);
    const list = activeRuleTab === 'always' ? moduleState.alwaysTerms : moduleState.blockedTerms;
    ...
    for (const termText of list) {
      ...
      remove.setAttribute('aria-label', `Remove ${termText}`);
  ...
  function renderCoupons() {
    ...
    for (const item of filtered.slice(0, 60)) {
      ...
      if (item.status === 'clipped') meta.textContent = 'Clipped';
      else if (item.status === 'ambiguous') meta.textContent = 'Needs review · not actionable';
      else if (!item.decision.eligible) meta.textContent = `Blocked by “${item.decision.reason}”`;
      else if (item.decision.reason?.startsWith('always:')) meta.textContent = `Always clip · ${item.decision.reason.replace('always: ', '')}`;
      else meta.textContent = 'Eligible';

      const reveal = () => item.element?.scrollIntoView({ behavior: 'smooth', block: 'center' });
  ...
  function renderActivity() {
    ...
    for (const entry of activity.slice(0, 14)) {
      const row = document.createElement('div');
      row.className = `activityRow${entry.level !== 'info' ? ` ${entry.level}` : ''}`;
  ...
  function renderRunControls() {
    ...
    const canStart = Boolean(activeModule && health?.available > 0);
    $('.start').disabled = running || !canStart;
  ...
  function render() {
    ...
    $('.minimize').setAttribute('aria-label', state.minimized ? 'Expand Coupon Pilot' : 'Minimize Coupon Pilot');
  }
  ...
  $('.dryRun').addEventListener('change', async event => {
    ...
    await persist();
    if (!state.automation.dryRun) clearPreviewMarks();
    renderRunControls();
  });
  ...
  GM_registerMenuCommand('Coupon Pilot: Clear preview marks', () => {
    clearPreviewMarks();
    toast('Preview marks cleared.');
  });

  const observer = new MutationObserver(() => scheduleRefresh(220));
  observer.observe(document.body, { childList: true, subtree: true });

  window.addEventListener('popstate', () => scheduleRefresh(250));
